...
ap.add_argument('--gridwidth', help='Width of chessboard grid (from intersection to adjacent intersection) in mm. Default = 1')
...
ap.add_argument('--path', help='Path (relative from script) to store results. Default = /')
...
print(f'Chessboard grid width: {grid_width} mm')
...
while verify:
	...
	if ret_l is True and ret_r is True:
		...
		frame_l = cv.putText(frame_l, f'({int(obj_dist_l[0][0][0])}, {int(obj_dist_l[0][0][1])}, {int(obj_dist_l[0][0][2])})', (int(point_l[0][0][0]), int(point_l[0][0][1])), cv.QT_FONT_NORMAL, 0.5, (0, 255, 0), 1, cv.LINE_AA)

	disp_frame = frame_l.copy()
